...
public class GoodHotListAdapter extends BaseAdapter {

    private List<GoodsHotListBean> mList;
    private Context mcontext;

    public GoodHotListAdapter(Context context, List<GoodsHotListBean> list) {
        mcontext = context;
        mList = list;
    }
    @Override
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup) {
        ...
        if (convertView == null) {
            ...
            convertView = View.inflate(mcontext, R.layout.item_goos, null);
            ...
            viewHolder.tv_content_text = (TextView) convertView.findViewById(R.id.tv_content_text);
            viewHolder.tv_good_price_text = (TextView) convertView.findViewById(R.id.tv_good_price_text);
            convertView.setTag(viewHolder);
        } else {
            viewHolder = (ViewHolder) convertView.getTag();
        }
       //Picasso.with(mcontext).load(mList.get(position).getGood_header()).config().into(viewHolder.icon_pic);
      ...
      //  Picasso.with(mcontext).load("https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1513102405146&di=e35096fba3d3b90e5c9b6a5fad5c2956&imgtype=0&src=http%3A%2F%2Fimage.tianjimedia.com%2FuploadImages%2F2017%2F319%2F18%2F450HM9U7G69F_600.jpg").config(Bitmap.Config.RGB_565).into(viewHolder.icon_pic);
        viewHolder.icon_title.setText(mList.get(position).getGood_name()+ "...");
        viewHolder.tv_content_text.setText(mList.get(position).getContent());
